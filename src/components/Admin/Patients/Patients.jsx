...
import "./Doctors.css";
import { useAdminGetDoctorQuery, useAdminGetPatientQuery } from "../../../redux/api/adminApi";
...
const Patients = () => {
  ...
  const { data, isLoading, refetch } = useAdminGetPatientQuery(query);
  ...
  const doctorColumns = [
    ...
    },
    {
      title: "email",
      width: 10,
      render: (data) => <h6 className="text-nowrap mb-0">{data?.email}</h6>,
      ellipsis: true,
    },
    {
      title: "Địa chỉ",
      width: 20,
      render: (data) => (
        <h6 className="text-nowrap mb-0">{data?.address}</h6>
      ),
      ellipsis: true,
    },
    {
    ...
    <>
      <AdminLayout>
        ...
        <div className="preTable d-flex justify-content-between">
          ...
          >
            <FaPlusCircle /> <span>Thêm bệnh nhân</span>
          </div>
        </div>
